...
def signin():
    ...
    sql = "SELECT * FROM `member` WHERE `user_id` = '"+check1+"' AND `user_password` = '"+check2+"'"
    cursor.execute(sql)
...
def member():
    if "user" in session:
        check1=session["user"]
        sql = "SELECT `user_name` FROM `member` WHERE `user_id` = '"+check1+"'"
        cursor.execute(sql)
        result=cursor.fetchone()
        result_ok=str(result).replace("[",'').replace("]",'').replace("(",'').replace(")",'').replace("'",'').replace(",",'')
        return render_template(
            "member.html",
            name=result_ok
        )
